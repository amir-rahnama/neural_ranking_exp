...
#device = 'GPU:0'
start = time.time()
# Load the SavedModel within the GPU context
...
class Sampling_generator:
    ...
    def dlime_sampling(self, instance_explained, qid_data):
        ...
        clustering = AgglomerativeClustering(self.num_clust(cleaned_docs)).fit(cleaned_docs)
...
class Explanations:
    ...
    def get_explanations(self, qid_data, position_rank, sampling):
        ...
        '''try:
            ...
            return gam_explanations, np.zeros((len(_name_features)))'''

        '''ros = RandomOverSampler(random_state=42, sampling_strategy='minority')
        samples_fea = [series[2:] for series in generated_docs]
        X_resampled, y_resampled = ros.fit_resample(np.array(samples_fea), labels)'''
        
        X_resampled = samples_fea
        y_resampled = labels
...
def main():
    ...
    #qids = [28203, 28587, 23068, 28700, 24240, 23890, 28483, 23132, 29379, 23022, 24923, 25000, 29889, 25191, 25207, 25226,
    #        28035, 28670, 28286, 28276]
    ...
    for sample_size in [500, 1000, 2000, 3000, 5000]: 
        qids_dict[sample_size] = {}
        print('sample_size', sample_size)
        for strategy in ['smote', 'gaussian', 'lhs', 'dlime']:
            qids_dict[sample_size][strategy] = {} 
            print('strategy', strategy)
            sampling = Sampling_generator(df_test, sample_size=sample_size, strategy_name=strategy)
            
            qids_dict[sample_size][strategy]['eval'] = {'imbalance': [], 'rbo': [], 'overlap': []}
            qids_dict[sample_size][strategy]['exp'] = []
            qids_dict[sample_size][strategy]['gt'] = []
            ...
            for key in qids:
                ...
                if isinstance(lime_exp, np.ndarray):
                    qids_dict[sample_size][strategy]['exp'].append(lime_exp)
                else: 
                    qids_dict[sample_size][strategy]['exp'].append(lime_exp.numpy().flatten())
                
                qids_dict[sample_size][strategy]['gt'].append(gam_exp.numpy().flatten())
                
                '''rbo_m, overlap_m = calculate_metrics(gam_exp, lime_exp, 0.95)
                
                qids_dict[sample_size][strategy]['eval']['imbalance'].append(exp.ratio_zeros)
                qids_dict[sample_size][strategy]['eval']['rbo'].append(rbo_m)
                qids_dict[sample_size][strategy]['eval']['overlap'].append(overlap_m)'''
                
                #print('Finished ' + strategy + ' with qid: ' + str(key))
            test.append(qids_dict)

        pickle.dump(qids_dict, open( "./laura_exps_{}_v3_rank_{}.p".format(sample_size, ranked_pos), "wb" ) )
